function  gen_random_stimvec(vec_dur,stim_dur, num_stims, amplitude, fs, num_fs, figf)
...
min_dist=0.5*fs;           %minimum distance
...
Rn=points_space*Ro(1:num_stims)/sum(Ro); %normalize
%spacing of points
...
% 
fprintf(fid,'%s\t%s\n %s\t%s\n %s\n %s\n %s\n %s\n %s\n %s\n %s\n %s\t%s\n %s\t%s\n', 'ATF',	'1.0','0', '6     ', '"AcquisitionMode=Episodic Stimulation"',  '"Comment= Created by tycho.hoogland@gmail.com"', '"YTop=5"',  '"YBottom=0"', '"SyncTimeUnits=10"', '"SweepStartTimesMS=0.000"', '"SignalsExported=OUT0"', '"Signals= "',	'"V"', '"Time (s)"','"OUT0 (V)"');
% %fprintf(fid,'%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n', 'ATF	1.0','8 3', '"AcquisitionMode=Episodic Stimulation"',  '"Comment="', '"YTop=0,0"',  '"YBottom=0,0"', '"SyncTimeUnits=10"', '"SweepStartTimesMS=0.000"', '"SignalsExported=IN 1"', '"Signals="	"IN 1"', '"Time (s)" "Trace #1 (mV)"');
fprintf(fid,'%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8\n',[x; stimvecs; stimvecs; stimvecs; stimvecs; stimvecs]);
